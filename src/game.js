...
const C = {
  base: 0.08,
  key: 'aegean-fleet-fleet-v6',
  old: 'aegean-fleet-fleet-v5',
...
function fresh() {
  return { m: 0, s: 1, cash: C.startCash, unlocked: ['ayvalik', 'mytilene'], openedLines: [], speedTiers: [1, 2], vehicles: [newVehicle('v1')], selected: 'v1', w: emptyQueues(), fr: {}, v: 6, marketTab: 'vehicles' };
}
...
const E = {
  ...
  unlock: q('unlockPort'), fareText: q('fareText'), fullness: q('fullness'), fullnessText: q('fullnessText'), autoTour: q('autoTour'),
...
function demandScore(a, b, date = now()) {
  const path = findTransferPath(a, b);
  if (!path) return 0;
  const distance = path.slice(0, -1).reduce((sum, port, index) => sum + route(port, path[index + 1]).distanceKm, 0);
  return seasonBand(date)[2] * timeBand(date)[1] * (ports[a].intensity || 0.6) * Math.max(0.55, Math.min(1.15, 70 / distance));
}
...
function portIcon(id, index) {
  const locked = !unlocked(id), side = index % 2 ? 'l' : 'r';
  return L.divIcon({ className: '', html: `<div class="ps ${side} ${locked ? 'locked' : ''}"><div class="pd"></div><div class="pc"><div class="pt">${ports[id].name}${locked ? ' · Kilitli' : ''}</div><div class="pv" id="pv-${id}"></div></div></div>`, iconSize: [18, 18], iconAnchor: [9, 9] });
}
PORT_IDS.forEach((id, index) => markers[id] = L.marker(ports[id].terminal, { icon: portIcon(id, index), zIndexOffset: 400 }).addTo(map));
...
function board(v) {
  ...
  const candidates = S.unlocked
    .filter(finalDestination => finalDestination !== v.loc && queueCount(v.loc, finalDestination) > 0 && nextHop(v.loc, finalDestination) === v.dst)
    .sort((a, b) => Number(a !== v.dst) - Number(b !== v.dst));
...
function releaseBoarding(v) {
  if (v.st !== 'boarding') return false;
  let returned = 0;
  for (const [finalDestination, count] of Object.entries(v.manifest || {})) {
    if (count <= 0) continue;
    setQueue(v.loc, finalDestination, queueCount(v.loc, finalDestination) + count);
    returned += count;
  }
  ...
  v.st = 'idle';
  refreshRouteLine();
  toast(returned ? `${v.name}: sefer iptal edildi · ${returned} yolcu limana döndü` : `${v.name}: otomatik tur kapatıldı · gemi serbest`);
  return true;
}
function depart(v) {
...
function step(gameMinutes) {
  ...
  for (const v of S.vehicles) {
    if (v.st === 'boarding') { S.cash -= gameMinutes * C.dockTax; board(v); }
      ...
      const loadRatio = v.pas / spec(v).capacity;
      const cost = distance * spec(v).opKm * (1 + C.loadCostMax * loadRatio);
      v.currentOp += cost; S.cash -= cost;
      if (v.p >= 1) arrive(v);
    }
  }
  const minute = Math.floor(S.m);
  if (minute !== lastSaveMinute && minute % 5 === 0) { save(); lastSaveMinute = minute; }
}
...
function updatePortHover() {
  for (const id of PORT_IDS) {
    ...
    for (const next of outgoing) {
      const finals = S.unlocked.filter(finalDestination => finalDestination !== id && nextHop(id, finalDestination) === next);
      const total = finals.reduce((sum, finalDestination) => sum + queueCount(id, finalDestination), 0);
      const transfers = finals
        .filter(finalDestination => finalDestination !== next && queueCount(id, finalDestination) > 0)
        .map(finalDestination => `${queueCount(id, finalDestination)} ${ports[finalDestination].name}`);
      rows.push(`<div><strong>${total}</strong> yolcu ${ports[next].name} yönü için bekliyor.</div>`);
      if (transfers.length) rows.push(`<div class="transfer-breakdown"><strong>Aktarma:</strong> ${transfers.join(', ')}</div>`);
    }
...
function shipCards() {
  const spawn = S.unlocked.map(id => `<option value="${id}">${ports[id].name}</option>`).join('');
  return Object.entries(TYPES).map(([id, type]) => `<div class="market-card"><h3>${type.name}</h3><div class="market-meta">Kapasite: ${type.capacity} · Hız: ${type.speed} km/sa · Menzil: ${type.range} km · Boş operasyon: €${type.opKm.toFixed(2)}/km</div><label class="market-spawn">Başlangıç limanı<select data-spawn="${id}">${spawn}</select></label><button data-buy="${id}" ${S.cash < type.price ? 'disabled' : ''}>${S.cash >= type.price ? `${money(type.price)} karşılığında satın al` : `${money(type.price - S.cash)} daha gerekli`}</button></div>`).join('');
}
function lineCards() {
  const open = S.openedLines.includes('chios-mytilene'), ready = unlocked('chios'), cost = 30000;
  return `<div class="market-card"><h3>Sakız ↔ Midilli</h3><div class="market-meta">Doğrudan deniz hattı · açıldığında aktarmalı yolcular direkt sefere geçer.</div><button data-buy-line="chios-mytilene" ${open || !ready || S.cash < cost ? 'disabled' : ''}>${open ? 'Hat açık' : !ready ? 'Önce Sakız hattını aç' : S.cash < cost ? `${money(cost - S.cash)} daha gerekli` : `${money(cost)} karşılığında hattı aç`}</button></div>`;
}
function progressCards() {
  return Object.entries(SPEED_UPGRADES).map(([tier, upgrade]) => {
    const owned = S.speedTiers.includes(Number(tier));
    return `<div class="market-card"><h3>${tier}× oyun hızı</h3><div class="market-meta">Simülasyonu ${tier} kat hızda çalıştırır. Temel zaman: 1 gerçek saniye = 1 oyun dakikası.</div><button data-buy-speed="${tier}" ${owned || S.cash < upgrade.cost ? 'disabled' : ''}>${owned ? 'Satın alındı' : S.cash < upgrade.cost ? `${money(upgrade.cost - S.cash)} daha gerekli` : `${money(upgrade.cost)} karşılığında aç`}</button></div>`;
  }).join('');
}
function renderMarket() {
  document.querySelectorAll('.market-tab').forEach(button => button.classList.toggle('active', button.dataset.marketTab === S.marketTab));
  E.marketCards.innerHTML = S.marketTab === 'vehicles' ? shipCards() : S.marketTab === 'lines' ? lineCards() : progressCards();
  document.querySelectorAll('[data-buy]').forEach(button => button.onclick = () => buyShip(button.dataset.buy, document.querySelector(`[data-spawn="${button.dataset.buy}"]`)?.value));
  document.querySelectorAll('[data-buy-line]').forEach(button => button.onclick = () => buyLine(button.dataset.buyLine));
  document.querySelectorAll('[data-buy-speed]').forEach(button => button.onclick = () => buySpeed(Number(button.dataset.buySpeed)));
}
function buyShip(typeId, spawn) {
  const type = TYPES[typeId];
  if (!type || !spawn || !unlocked(spawn) || S.cash < type.price) return;
  S.cash -= type.price;
  const number = S.vehicles.length + 1, id = `v${number}`;
  S.vehicles.push(newVehicle(id, typeId, typeId === 'swift80' ? `MV Hızır ${number}` : `MV Ege ${number}`, spawn));
  S.selected = id; rebuildShips(); showView('fleet'); render(); save(); toast(`${S.vehicles.at(-1).name}, ${ports[spawn].name} limanında filoya katıldı.`);
}
function buyLine(id) {
  if (id !== 'chios-mytilene' || !unlocked('chios') || S.openedLines.includes(id) || S.cash < 30000) return;
  S.cash -= 30000; S.openedLines.push(id); save(); location.reload();
}
function buySpeed(tier) {
  const upgrade = SPEED_UPGRADES[tier];
  if (!upgrade || S.speedTiers.includes(tier) || S.cash < upgrade.cost) return;
  S.cash -= upgrade.cost; S.speedTiers.push(tier); renderMarket(); render(); save(); toast(`${tier}× oyun hızı açıldı.`);
}
...
function render() {
  ...
  E.fullness.value = v.fullness; E.fullnessText.textContent = `%${v.fullness}`; E.autoTour.checked = v.autoTour; E.rangeBadge.textContent = `Menzil ${type.range} km`; E.goal.textContent = v.fullness === 0 ? 'Hedef: ilk yolcu' : `Hedef: ${target(v)} yolcu`; E.progressLabel.textContent = `Kalkış hedefi: %${percentage}`; E.fill.style.width = `${Math.min(100, percentage)}%`;
  for (const ship of S.vehicles) shipMarkers[ship.id].setLatLng(ship.st === 'sailing' ? position(ship) : ports[ship.loc].terminal);
  destinationOptions();
  ...
  document.querySelectorAll('[data-s]').forEach(button => {
    const tier = Number(button.dataset.speedTier || 0), allowed = !tier || S.speedTiers.includes(tier);
    button.disabled = !allowed;
    button.title = allowed ? '' : `${tier}× hız Marketspace > İlerlemeler bölümünden açılır`;
    button.classList.toggle('a', Number(button.dataset.s) === S.s);
  });
}

E.destination.onchange = () => { vehicle().plannedDst = E.destination.value; render(); save(); };
...
E.autoTour.onchange = () => {
  const v = vehicle();
  v.autoTour = E.autoTour.checked;
  if (!v.autoTour) {
    v.pair = null;
    if (v.st === 'boarding') releaseBoarding(v);
  } else if (v.st === 'boarding' && v.dst) {
    v.pair = [v.loc, v.dst];
  }
  render(); save();
};
...
document.querySelectorAll('[data-s]').forEach(button => button.onclick = () => { if (button.disabled) return; S.s = Number(button.dataset.s); render(); save(); });
document.querySelectorAll('.navtab').forEach(button => button.onclick = () => showView(button.dataset.view));
document.querySelectorAll('.market-tab').forEach(button => button.onclick = () => { S.marketTab = button.dataset.marketTab; renderMarket(); save(); });
...
function load() {
  try {
    ...
    S.openedLines = parsed.openedLines || [];
    S.speedTiers = parsed.speedTiers || [1, 2];
    if (![0, 1, 2, 4, 8].includes(S.s)) S.s = 1;
    S.v = 6;
  } catch { S = fresh(); }
}
...
function loop(timestamp) {
  const realSeconds = Math.min(0.25, (timestamp - lastFrame) / 1000);
  lastFrame = timestamp;
  if (S.s > 0) step(realSeconds * S.s);
  render();
